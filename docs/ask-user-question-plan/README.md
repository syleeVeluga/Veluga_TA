...
이 폴더는 단일 문서였던 [`docs/ask-user-question-plan.md`](../ask-user-question-plan.md)를 **개요 / 단계별 상세 구현 / 검증**으로 분리한 것이다. 각 단계 문서는 실제 코드 경로·라인·재사용 패턴을 담아 그대로 실행 가능하게 작성했다(라인 번호는 *구현 시 재확인* 전제).
