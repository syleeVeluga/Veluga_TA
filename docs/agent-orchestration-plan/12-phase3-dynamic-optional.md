...
- [ ] 정적 그래프에 **조건부 엣지**를 추가(LangGraph 차용): 워커 결과 상태에 따라 후속 워커를 동적 선택(예: KB 결과 부족 시 file-analysis 범위 확대).
- [ ] 엣지 조건은 **순수 함수(상태→다음)** 로 구현, LLM 자유 생성 토폴로지는 아직 도입하지 않음.
- [ ] 재계획 전이 `RUNNING_PARALLEL → PLANNING` 활용(부분 실패/근거 부족 후 1회 재계획, 무한 루프는 스텝 예산으로 차단).
...
- [ ] cowork `AgentRuntimeExtension`로 **경계가 명확한 sub-session**만 기동(개수 상한, 토큰 예산 분할, 명시적 objective/boundaries).
- [ ] sub-session 결과는 **요약만** 부모로 반환(컨텍스트 격리, Claude Agent SDK 모델).
- [ ] 비용 가드 강화: 멀티에이전트 ~15× 토큰을 감안한 하드 상한 + 효과 측정(A/B).
- [ ] 워커 간 비통신 원칙 유지(모든 조정은 오케스트레이터).
...
- [ ] LLM이 토폴로지를 생성하는 경우에도 **반드시** `validate()`(사이클/누락/`toolScope` 검증)를 통과해야 실행.
- [ ] 생성 plan은 정책 화이트리스트로 sanitize, 신뢰도 낮으면 정적 폴백.
- [ ] 관측성/예산/체크포인트는 Phase 1/2 그대로 적용.
