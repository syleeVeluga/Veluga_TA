...
이 폴더는 원래 단일 문서였던 설계 초안을 **개요 / 배경·조사 / 갭 분석 / 아키텍처 / 단계별 상세 구현 / 검증**으로 분리한 것이다.
...
1. LLM 에이전트 루프는 base `pi-coding-agent`(cowork-core clone snapshot, 수정 금지)가 소유한다. Veluga는 `AgentRuntimeExtension` 훅 + `ToolDefinition.execute` 래퍼로만 끼어든다.
