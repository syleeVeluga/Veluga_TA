...
- 실제 루프는 base `pi-coding-agent`가 [packages/cowork-core/src/main/claude/agent-runner.ts](packages/cowork-core/src/main/claude/agent-runner.ts) 단일 세션에서 소유. cowork-core는 clone snapshot(수정 금지), Veluga는 `AgentRuntimeExtension` 훅으로만 통합 ([docs/reference/cowork-hooks.md](docs/reference/cowork-hooks.md)).
