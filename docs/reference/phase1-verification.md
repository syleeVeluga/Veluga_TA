...
| Veluga Mode OFF regression | Gap recorded | Run the packaged app with Veluga Mode OFF and execute the Open Cowork smoke flow. Expected result: parity with base behavior. Current automated coverage checks bindings and metadata only. |
...
- Open Cowork MIT attribution remains in `packages/veluga-ui/credits/LICENSES.md` and `packages/cowork-core/LICENSE` (CI-guarded).
