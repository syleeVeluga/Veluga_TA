...
Baseline source: `packages/cowork-core` at base commit `d4318943fb070d0863bed930eb70a95c6e7c4487`.

## Confirmed Integration Points

| Veluga need | Base hook candidate | Status | Integration note |
|---|---|---|---|
...
- Add a minimal base shim only where Open Cowork lacks an extension point.
...
The only material hook gap is the lack of a named, exported `beforeToolCall` hook. The practical hook is the tool wrapper stage inside `src/main/claude/agent-runner.ts`, before tools are passed into `createAgentSession(...)`. Track this until a Veluga shim or base extension API is chosen.
